...
- (NSArray *)getPersonInfoArray
{
    ...
    [contactStore enumerateContactsWithFetchRequest:request error:nil usingBlock:^(CNContact * _Nonnull contact, BOOL * _Nonnull stop) {
        ...
        for (CNLabeledValue *labeledValue in phoneNums) {
            // 一个人  不论几个号码  都将这个人的信息（除手机号不同外） 全部再复制一份进行保存personArray
...
//            personEntity.firstname = firstname;
            ...
            NSMutableString *fullname = [[NSString stringWithFormat:@"%@%@",contact.familyName,contact.givenName] mutableCopy];
...
//            personEntity.fullname = fullname;
            ...
            NSString *phoneValue = [phoneNumer valueForKey:@"digits"];//phoneNumer.stringValue;
            if (phoneValue.length > 0) {
//                personEntity.phoneNumber = phoneValue;
//                [personArray addObject:personEntity];
                NSMutableDictionary *infoDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:fullname,@"fullname",phoneValue,@"phoneNumber",@"0",@"isHidden", nil];
                [personArray addObject:infoDic];
            }
        }
    }];
    return personArray;
